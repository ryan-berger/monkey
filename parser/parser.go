...
func New(lexer lexer.Lexer) *Parser {
	...
	p.prefixParserFns = make(map[token.Type]prefixParserFn)
	...
	p.infixParserFns = make(map[token.Type]infixParserFn)
...
func (p *Parser) registerPrefix(tokenType token.Type, fn prefixParserFn) {
	p.prefixParserFns[tokenType] = fn
}

func (p *Parser) registerInfix(tokenType token.Type, fn infixParserFn) {
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		msg := fmt.Sprintf("could not nparse %q as integer", p.curToken.Literal)
		p.errors = append(p.errors, msg)
		return nil
	}
